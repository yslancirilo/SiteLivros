...
export default function BookCard({ book, onBuyClick, onBookClick }) {
  const [imgError, setImgError] = useState(false);

  const formattedPrice = book.price.toFixed(2).replace(".", ",");

  return (
    <article className="book-card" aria-label={`Livro: ${book.title}`}>
      {/* Book cover */}
      <button
      ...
      <div className="card-body">
        ...
        <p className="card-condition">{book.condition}</p>
        ...
        <p className="price-shipping">+ frete a combinar</p>
